...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self, inputs, reconstructions, global_step=None, posteriors=None, optimizer_idx=None,
                 last_layer=None, cond=None, split="train",
                weights=None, return_dic=False):
        rec_loss = torch.abs(inputs.contiguous() - reconstructions.contiguous())
        rec_loss_copy = torch.mean(rec_loss) / rec_loss.shape[0]
        ...
        if self.perceptual_weight > 0:
            return weighted_nll_loss, rec_loss_copy, torch.mean(p_loss)
        else:
