...
from utils.image_transforms import Augment_RGB_torch
...
def myPSNR(tar_img, prd_img):
    ...
    prd_img = pil_to_tensor(prd_img)
    prd_img = prd_img / 255.0
...
def log_validation(val_dataloader, vae, text_encoder, tokenizer, unet, noise_scheduler, args, accelerator, weight_dtype, epoch):
    ...
    for i, val_data in enumerate(val_dataloader):
        with torch.no_grad():
            ...
            with autocast_ctx:
                gt_image = val_data['pixel_values']
                condition_image = val_data['conditioning_pixel_values']
                batch_size = condition_image.shape[0]
                validation_prompts = args.validation_prompts * batch_size

                height, width = gt_image.shape[-2:]

                out_images = pipeline(condition_image, validation_prompts, num_inference_steps=20, generator=generator).images
                out_images = [image.crop((0, 0, width, height)) for image in out_images]
    ...
    logger.info("[Ep %d \t PSNR SIDD: %.4f\t ] " % (epoch, avg_psnr))

    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            np_images = np.stack([np.asarray(img) for img in images])
            tracker.writer.add_images("validation", np_images, epoch, dataformats="NHWC")
            tracker.writer.add_scalar("validation_psnr", avg_psnr, epoch)
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def preprocess_train(examples):
        ...
        scaled_ps = args.resolution
        for i in range(len(images)):
            augment.rotate = random.randint(-20, 20)
            apply_trans = transforms_aug[random.randint(0, 8)]  # image augment
            images[i] = getattr(augment, apply_trans)(images[i])
            conditioning_images[i] = getattr(augment, apply_trans)(conditioning_images[i])
            ...
            if H-scaled_ps!=0 or W-scaled_ps!=0:
                ...
                images[i] = images[i] [:, r:r + scaled_ps, c:c + scaled_ps]
                conditioning_images[i] = conditioning_images[i] [:, r:r + scaled_ps, c:c + scaled_ps]
...
def main():
    ...
    # do not load unet pretrained model
    # unet_config_path = "unet/config.json"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                condition_latents = vae.encode(batch["conditioning_pixel_values"].to(weight_dtype)).latent_dist.sample()
                ...
                # set new num_train_timesteps
                if args.num_train_timesteps is not None:
                    noise_scheduler.register_to_config(num_train_timesteps=args.num_train_timesteps)
                ...
                # concat
                noisy_latents = torch.cat((noisy_latents, condition_latents), dim=1)
                ...
                batch_loss = accelerator.gather(loss.repeat(args.train_batch_size))
