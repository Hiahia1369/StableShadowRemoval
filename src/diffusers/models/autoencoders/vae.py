...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        sample_size = sample_size // (2 ** (len(block_out_channels) - 1))
            ...
            for i, up_block in enumerate(self.up_blocks):
                if self.add_cfw and enc_features != None:
                    ...
                    if dino != None and sample.shape[2] != dino.shape[2]:
                        dino = F.upsample_bilinear(dino, scale_factor=2)
                    sample = up_block(sample, latent_embeds, enc_features=enc_features[len(self.up_blocks)-1-i], dino=dino)
                else:
                    sample = up_block(sample, latent_embeds)

        # post-process
        if latent_embeds is None:  # None
            sample = self.conv_norm_out(sample)
        else:
            sample = self.conv_norm_out(sample, latent_embeds)
        sample = self.conv_act(sample) #(b, 128, 512, 512)
        sample = self.conv_out(sample) #(b, 3, 512, 512)
