...
class AutoencoderKL(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        if self.add_x_feature:
            ...
            decoder_in_channel = latent_channels
        ...
        self.post_quant_conv = nn.Conv2d(decoder_in_channel, decoder_in_channel, 1)
    ...
    def _decode(self, z: torch.Tensor, enc_features: torch.Tensor = None,
                x: torch.Tensor = None,
                dino: torch.Tensor = None, point: torch.Tensor = None,
        ...
        if x is None:
            dec = self.decoder(z, enc_features=enc_features, dino=dino)
        else:
            dec = self.decoder(z, enc_features=enc_features, x=x, point=point, normal=normal)
    ...
    @apply_forward_hook
    def decode(self, z: torch.Tensor, enc_features:torch.Tensor = None, 
               x: torch.Tensor = None, 
               dino: torch.Tensor = None, point: torch.Tensor = None,
               normal: torch.Tensor = None, 
        ...
        if self.use_slicing and z.shape[0] > 1:
            ...
            decoded = self._decode(z, enc_features=enc_features, x=x, dino=dino, point=point, normal=normal, return_dict=False)[0]
